...
#' @useDynLib sudachir2, .registration = TRUE
#' @keywords internal
NULL
...
#' @export
`$<-.savvy_sudachir2__sealed` <- function(x, name, value) {
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}

#' @export
`[[<-.savvy_sudachir2__sealed` <- function(x, i, value) {
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}
...
`tagger_inner` <- function(`x`, `mode`, `config_file`, `resource_dir`, `dictionary_path`) {
  .Call(savvy_tagger_inner__impl, `x`, `mode`, `config_file`, `resource_dir`, `dictionary_path`)
}
